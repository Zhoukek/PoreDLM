...
def _extract_hidden(output, *names: str) -> torch.Tensor | None:
    for name in names:
        hidden = output.get(name) if isinstance(output, dict) else getattr(output, name, None)
...
def get_context_hidden(model, input_ids: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
    # New and old exports both usually expose the context encoder directly.
    if hasattr(model, "context_encoder"):
        ...
        hidden = _extract_hidden(out, "last_hidden_state")
        if hidden is not None:
            return hidden

    # Fallback for exports that only expose context_hidden_state through forward().
    try:
        out = model(input_ids=input_ids, attention_mask=attention_mask, return_context=True, return_dict=True)
        hidden = _extract_hidden(out, "context_hidden_state", "last_hidden_state")
        if hidden is not None:
            return hidden
    except TypeError:
        pass

    raise ValueError("Cannot find context hidden state from HF PoreLM output.")


def flow_t_eps(model) -> float:
    cfg = getattr(model, "config", None)
    dlm_cfg = getattr(cfg, "dlm_config", None) or {}
    return float(dlm_cfg.get("t_eps", 0.05))


def flow_noise_scale(model) -> float:
    cfg = getattr(model, "config", None)
    dlm_cfg = getattr(cfg, "dlm_config", None) or {}
    return float(dlm_cfg.get("denoiser_noise_scale", 1.0))


def denoiser_net_out_to_v_x(net_out, z: torch.Tensor, t: torch.Tensor, t_eps: float) -> tuple[torch.Tensor, torch.Tensor]:
...
def get_denoiser(model):
    denoiser = getattr(model, "flow_denoiser", None)
    if denoiser is None:
        denoiser = getattr(model, "elf_denoiser", None)
    return denoiser
...
def ode_from_context_hidden(
    ...
    # New HF exports provide the exact ODE helper used by the exported model.
    if hasattr(model, "ode_from_context_hidden"):
        return model.ode_from_context_hidden(
            context,
    ...
    if denoiser is None:
        raise ValueError("HF DLM does not expose flow_denoiser or elf_denoiser; cannot compute ode_hidden.")
    context = context.to(dtype=next(denoiser.parameters()).dtype)
    z = context
    ...
    for idx in range(int(ode_steps)):
        ...
        num_self_cond = int(getattr(denoiser, "num_self_cond_cfg_tokens", 0))
        if num_self_cond > 0:
            ...
            net_out = denoiser(z, t_batch, attention_mask=attention_mask, decoder_step_active=False)
        v_pred, x_pred = denoiser_net_out_to_v_x(net_out, z, t_batch, flow_t_eps(model))
        z = z + (t_next - t) * v_pred
...
def sde_from_context_hidden(
    ...
    # Prefer the helper embedded in newer HF exports when available.
    if hasattr(model, "sde_from_context_hidden"):
        return model.sde_from_context_hidden(
            context,
            attention_mask=attention_mask,
    ...
    denoiser = get_denoiser(model)
    if denoiser is None:
        raise ValueError("HF DLM does not expose flow_denoiser or elf_denoiser; cannot compute sde_hidden.")
    if float(sde_gamma) < 0.0:
        raise ValueError("--sde-gamma must be >= 0.")

    context = context.to(dtype=next(denoiser.parameters()).dtype)
    z = context
    x_pred = torch.zeros_like(z)
    t_steps = torch.linspace(float(sde_start_t), 1.0, int(sde_steps) + 1, device=z.device, dtype=z.dtype)
    generator = None
    if seed is not None:
        generator = torch.Generator(device=z.device)
        generator.manual_seed(int(seed))

    for idx in range(int(sde_steps)):
        t = t_steps[idx]
        t_next = t_steps[idx + 1]
        h = t_next - t
        alpha = torch.clamp(1.0 - float(sde_gamma) * h, min=0.0, max=1.0)
        t_back = alpha * t
        eps = torch.randn(z.shape, device=z.device, dtype=z.dtype, generator=generator) * flow_noise_scale(model)
        ...
        num_self_cond = int(getattr(denoiser, "num_self_cond_cfg_tokens", 0))
        if num_self_cond > 0:
            model_input = torch.cat([z_back, x_pred], dim=-1)
            cfg_scale = torch.full((z.shape[0],), float(self_cond_cfg_scale), device=z.device, dtype=z.dtype)
            net_out = denoiser(
                model_input,
                t_batch,
            ...
            net_out = denoiser(z_back, t_batch, attention_mask=attention_mask, decoder_step_active=False)
        v_pred, x_pred = denoiser_net_out_to_v_x(net_out, z_back, t_batch, flow_t_eps(model))
...
def sample_from_context_hidden(
    model,
    context: torch.Tensor,
    attention_mask: torch.Tensor,
    ...
    if args.sampling_method == "sde":
        return sde_from_context_hidden(
            model,
            context,
            attention_mask,
            args.sde_steps,
            args.sde_start_t,
            args.sde_gamma,
            args.self_cond_cfg_scale,
            args.sde_seed,
        )
    return ode_from_context_hidden(
        model,
        context,
        attention_mask,
        args.ode_steps,
...
def save_global_comparison_plot(
    ...
    marker_handles = [
        ...
        mlines.Line2D([], [], color="#555555", marker="^", linestyle="None", markersize=6, label=refined_label),
        mlines.Line2D([], [], color="#555555", marker="x", linestyle="None", markersize=6, label="context center"),
        mlines.Line2D([], [], color="#555555", marker="+", linestyle="None", markersize=7, label=f"{refined_label} center"),
    ]
    ax.legend(handles=marker_handles, frameon=False, loc="best")
    ax.set_title(f"LB06 all same-site reads: context_hidden and {refined_label}")
...
def save_site_comparison_plot(
    ...
    ax.set_title(f"{site}: context_hidden vs {refined_label}")
    ax.set_xlabel(f"PC1 ({explained[0] * 100:.1f}%)")
    ax.set_ylabel(f"PC2 ({explained[1] * 100:.1f}%)")
    ax.grid(True, linewidth=0.4, alpha=0.25)
    ...
    metrics_text = (
        ...
        f"{refined_label} RMS radius: {ode_metrics['centroid_rms_l2']:.4g}\n"
        f"{refined_label}/context RMS: {rms_ratio:.3g}\n"
        f"context mean pairwise L2: {context_metrics['pairwise_mean_l2']:.4g}\n"
        f"{refined_label} mean pairwise L2: {ode_metrics['pairwise_mean_l2']:.4g}\n"
        f"{refined_label}/context pairwise: {pairwise_ratio:.3g}\n"
        f"{refined_label}/context cosine: {cosine_ratio:.3g}"
    )
    ax.text(
        0.02,
        0.98,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sampling-method", choices=("ode", "sde"), default="ode", help="Refinement method from context_hidden.")
    parser.add_argument("--ode-steps", type=int, default=4)
    parser.add_argument("--ode-start-t", type=float, default=0.85)
    parser.add_argument("--sde-steps", type=int, default=4)
    parser.add_argument("--sde-start-t", type=float, default=0.85)
    parser.add_argument("--sde-gamma", type=float, default=0.1, help="SDE noise strength; 0 approaches deterministic ODE behavior.")
    parser.add_argument("--sde-seed", type=int, default=None, help="Optional fixed seed for reproducible SDE noise.")
...
def main() -> None:
    ...
    refined_label = f"{args.sampling_method}_hidden"
    print(f"[setup] sampling_method={args.sampling_method} refined_label={refined_label}")
    ...
    refined_vectors: list[np.ndarray] = []
    ...
    for item in iter_reads(input_dir, args.max_reads_per_site):
        ...
        with torch.inference_mode():
            ...
            refined = sample_from_context_hidden(dlm, context, attention_mask, args)
        context_vectors.append(mean_pool(context, attention_mask))
        refined_vectors.append(mean_pool(refined, attention_mask))
    ...
    refined_arr = np.concatenate(refined_vectors, axis=0)
    sites = [row.site for row in all_rows]

    context_xy, context_exp = pca_2d(context_arr)
    refined_xy, refined_exp = pca_2d(refined_arr)
    save_payload = {
        "context_hidden": context_arr,
        refined_label: refined_arr,
        "refined_hidden": refined_arr,
        "context_pca": context_xy,
        f"{args.sampling_method}_pca": refined_xy,
        "refined_pca": refined_xy,
        "context_pca_explained": context_exp,
        f"{args.sampling_method}_pca_explained": refined_exp,
        "refined_pca_explained": refined_exp,
    ...
    if args.sampling_method == "ode":
        save_payload["ode_hidden"] = refined_arr
        save_payload["ode_pca"] = refined_xy
        save_payload["ode_pca_explained"] = refined_exp
    np.savez_compressed(output_dir / "same_site_read_embeddings.npz", **save_payload)
    ...
    save_plot(refined_xy, sites, f"LB06 same-site reads: {refined_label} PCA", output_dir / f"{refined_label}_pca.png")
    global_joint_metrics = save_global_comparison_plot(
        context_arr,
        refined_arr,
        sites,
        output_dir / f"all_sites_context_vs_{refined_label}_pca.png",
    ...
    for site, indices in site_to_indices.items():
        ...
        site_refined = refined_arr[idx]
        context_metrics = cluster_metrics(site_context)
        refined_metrics = cluster_metrics(site_refined)
        plot_metrics = save_site_comparison_plot(
            site_context,
            site_refined,
            site,
            per_site_dir / f"{site}.context_vs_{refined_label}_pca.png",
            refined_label=refined_label,
        )
        row = {
            "site": site,
            ...
            "ode_centroid_mean_l2": refined_metrics["centroid_mean_l2"],
            "ratio_centroid_mean_l2": metric_ratio(refined_metrics["centroid_mean_l2"], context_metrics["centroid_mean_l2"]),
            "context_centroid_median_l2": context_metrics["centroid_median_l2"],
            "ode_centroid_median_l2": refined_metrics["centroid_median_l2"],
            "ratio_centroid_median_l2": metric_ratio(refined_metrics["centroid_median_l2"], context_metrics["centroid_median_l2"]),
            "context_centroid_rms_l2": context_metrics["centroid_rms_l2"],
            "ode_centroid_rms_l2": refined_metrics["centroid_rms_l2"],
            "ratio_centroid_rms_l2": metric_ratio(refined_metrics["centroid_rms_l2"], context_metrics["centroid_rms_l2"]),
            "context_pairwise_mean_l2": context_metrics["pairwise_mean_l2"],
            "ode_pairwise_mean_l2": refined_metrics["pairwise_mean_l2"],
            "ratio_pairwise_mean_l2": metric_ratio(refined_metrics["pairwise_mean_l2"], context_metrics["pairwise_mean_l2"]),
            "context_pairwise_median_l2": context_metrics["pairwise_median_l2"],
            "ode_pairwise_median_l2": refined_metrics["pairwise_median_l2"],
            "ratio_pairwise_median_l2": metric_ratio(refined_metrics["pairwise_median_l2"], context_metrics["pairwise_median_l2"]),
            "context_pairwise_mean_cosine": context_metrics["pairwise_mean_cosine"],
            "ode_pairwise_mean_cosine": refined_metrics["pairwise_mean_cosine"],
            "ratio_pairwise_mean_cosine": metric_ratio(refined_metrics["pairwise_mean_cosine"], context_metrics["pairwise_mean_cosine"]),
            "ode_more_compact_by_centroid_rms": bool(
                np.isfinite(context_metrics["centroid_rms_l2"])
                and np.isfinite(refined_metrics["centroid_rms_l2"])
                and refined_metrics["centroid_rms_l2"] < context_metrics["centroid_rms_l2"]
            ),
            **plot_metrics,
        }
    ...
    refined_separation = {
        "representation": refined_label,
        **site_separation_metrics(refined_arr, sites),
    }
    separation_comparison = {
        "within_site_rms_ratio_ode_over_context": metric_ratio(
            refined_separation["within_site_rms_weighted_mean"],
            context_separation["within_site_rms_weighted_mean"],
        ),
        "within_site_pairwise_ratio_ode_over_context": metric_ratio(
            refined_separation["within_site_pairwise_weighted_mean"],
            context_separation["within_site_pairwise_weighted_mean"],
        ),
        "between_site_center_l2_ratio_ode_over_context": metric_ratio(
            refined_separation["between_site_center_l2_mean"],
            context_separation["between_site_center_l2_mean"],
        ),
        "min_between_site_center_l2_ratio_ode_over_context": metric_ratio(
            refined_separation["between_site_center_l2_min"],
            context_separation["between_site_center_l2_min"],
        ),
        "separation_over_within_rms_ratio_ode_over_context": metric_ratio(
            refined_separation["separation_over_within_rms"],
            context_separation["separation_over_within_rms"],
        ),
        "min_separation_over_within_rms_ratio_ode_over_context": metric_ratio(
            refined_separation["min_separation_over_within_rms"],
            context_separation["min_separation_over_within_rms"],
        ),
    }
    separation_comparison.update(
        {
            "within_site_rms_ratio_refined_over_context": separation_comparison["within_site_rms_ratio_ode_over_context"],
            "within_site_pairwise_ratio_refined_over_context": separation_comparison["within_site_pairwise_ratio_ode_over_context"],
            "between_site_center_l2_ratio_refined_over_context": separation_comparison["between_site_center_l2_ratio_ode_over_context"],
            "min_between_site_center_l2_ratio_refined_over_context": separation_comparison["min_between_site_center_l2_ratio_ode_over_context"],
            "separation_over_within_rms_ratio_refined_over_context": separation_comparison["separation_over_within_rms_ratio_ode_over_context"],
            "min_separation_over_within_rms_ratio_refined_over_context": separation_comparison["min_separation_over_within_rms_ratio_ode_over_context"],
        }
    )
    global_separation_rows = [context_separation, refined_separation]
    ...
    summary = {
        ...
        f"{refined_label}_shape": list(refined_arr.shape),
        "refined_hidden_shape": list(refined_arr.shape),
        "context_pca_explained": context_exp.tolist(),
        f"{args.sampling_method}_pca_explained": refined_exp.tolist(),
        "refined_pca_explained": refined_exp.tolist(),
        "ode_steps": int(args.ode_steps),
        "ode_start_t": float(args.ode_start_t),
        "sde_steps": int(args.sde_steps),
        "sde_start_t": float(args.sde_start_t),
        "sde_gamma": float(args.sde_gamma),
        "sde_seed": args.sde_seed,
        "self_cond_cfg_scale": float(args.self_cond_cfg_scale),
        ...
        "global_joint_plot": str(output_dir / f"all_sites_context_vs_{refined_label}_pca.png"),
        **separation_comparison,
