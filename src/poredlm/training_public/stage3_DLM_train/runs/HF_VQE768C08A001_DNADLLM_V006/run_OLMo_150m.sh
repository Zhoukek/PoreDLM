...
RUN_ROOT=${PROJECT_ROOT}/src/poredlm/training_public/stage3_DLM_train/runs/HF_VQE768C08A001_DNADLLM_V006
...
case "${DLM_OBJECTIVE}" in
    ...
    *)
        echo "DLM_OBJECTIVE must be elf or bert" >&2
        exit 2
        ;;
esac
...
nohup torchrun --nproc_per_node=8 --rdzv_endpoint=localhost:29511 \
    scripts/train_DLM.py ${RUN_ROOT}/config_150m.yaml \
    --run_name="dlm_V004_${DLM_OBJECTIVE}_${DLM_TRAIN_MODE}" \
    --dlm.conditioning_mode="${DLM_TRAIN_MODE}" \
    --dlm.training_objective="${DLM_OBJECTIVE}" \
    --dlm.freeze_context_encoder="${FREEZE_CONTEXT}" \
    --wandb.entity="zhoukek-zhejiang-university" \
    --wandb.project="stage3_dlm_public" \
    --save_folder="${RUN_ROOT}/model_${DLM_OBJECTIVE}_${DLM_TRAIN_MODE}" \
    > "${RUN_ROOT}/nohup_${DLM_OBJECTIVE}_${DLM_TRAIN_MODE}.out" 2>&1 &
