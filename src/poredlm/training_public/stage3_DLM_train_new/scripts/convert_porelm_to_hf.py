...
PORELM_ROOT = Path(__file__).resolve().parents[1]
if str(PORELM_ROOT) not in sys.path:
    sys.path.insert(0, str(PORELM_ROOT))

FLOW_MATCHING_SRC = PORELM_ROOT / "src"
if FLOW_MATCHING_SRC.is_dir() and str(FLOW_MATCHING_SRC) not in sys.path:
    sys.path.insert(0, str(FLOW_MATCHING_SRC))

from porelm.config import TrainConfig  # noqa: E402
from porelm.model_porelm import PoreLM  # noqa: E402
...
class PoreLMConfig(PretrainedConfig):
    model_type = "porelm_diffusion"
    ...
    def __init__(
        ...
        flow_matching_src_path: Optional[str] = None,
        **kwargs: Any,
    ):
        ...
        self.flow_matching_src_path = flow_matching_src_path


class PoreLMForDiffusion(PreTrainedModel):
    config_class = PoreLMConfig
    base_model_prefix = "porelm"
    main_input_name = "input_ids"

    @staticmethod
    def _ensure_flow_matching_runtime(config: PoreLMConfig) -> None:
        candidate_paths = []
        model_dir = getattr(config, "name_or_path", None) or getattr(config, "_name_or_path", None)
        if model_dir:
            candidate_paths.append(Path(model_dir) / "flow_matching_core" / "src")
        candidate_paths.append(Path(__file__).resolve().parent / "flow_matching_core" / "src")
        if config.flow_matching_src_path:
            candidate_paths.append(Path(config.flow_matching_src_path))
    ...
    def __init__(self, config: PoreLMConfig):
        super().__init__(config)
        self._ensure_flow_matching_runtime(config)
        try:
            from porelm.flow_matching_core.model import FLOW_MATCHING_DENOISERS
        except ImportError as exc:
            raise ImportError(
                "Cannot import porelm.flow_matching_core. Install this package or add its src "
                "directory to PYTHONPATH before loading this HF model."
            ) from exc
        ...
        model_name = dlm.get("model", "flow_matching_b")
        if model_name not in FLOW_MATCHING_DENOISERS:
            raise ValueError(f"Unknown flow matching model {model_name!r}; expected one of {sorted(FLOW_MATCHING_DENOISERS.keys())}")
        self.flow_denoiser = FLOW_MATCHING_DENOISERS[model_name](
    ...
    def _flow_t_eps(self) -> float:
        return float(self.config.dlm_config.get("t_eps", 0.05))

    @staticmethod
    def _flow_net_out_to_v_x(
        net_out: Any,
        z: torch.Tensor,
        t: torch.Tensor,
        t_eps: float,
    ...
    def _flow_forward_ode_sample(
        ...
        num_self_cond_cfg_tokens = int(getattr(self.flow_denoiser, "num_self_cond_cfg_tokens", 0))
        ...
        return self._flow_net_out_to_v_x(net_out, z, t_batch, self._flow_t_eps())
    ...
    def _conditional_sample_step(
        ...
        v_cond, x_cond = self._flow_forward_ode_sample(
            z,
            t_batch,
            x_pred_prev,
            attention_mask=attention_mask,
        ...
        v_uncond, x_uncond = self._flow_forward_ode_sample(
            z_uncond,
            t_batch,
            x_prev_uncond,
            attention_mask=attention_mask,
    ...
    def generate(
        ...
        *,
        ...
        model_max_length = int(
            ...
            or self.flow_denoiser.max_length
        )
        ...
        if condition_token_mask is not None:
            ...
            # is only for the context encoder; the flow matching denoiser below still
        ...
        decoder_input = z
        if int(getattr(self.flow_denoiser, "num_self_cond_cfg_tokens", 0)) > 0:
        ...
        if logits is None:
            raise RuntimeError("flow matching decoder did not return logits.")
    ...
    def ode_from_context_hidden(
        ...
        *,
        ...
        for idx in range(ode_steps):
            ...
            v_pred, x_pred = self._flow_forward_ode_sample(
                z,
                t_batch,
                x_pred,
                attention_mask=attention_mask,
...
__all__ = ["PoreLMConfig", "PoreLMForDiffusion"]
'''
...
def _context_encoder_hf_config(model: PoreLM) -> tuple[str, Dict[str, Any]]:
    context_encoder = model.context_encoder
    if hasattr(context_encoder, "model"):
        wrapped_model = context_encoder.model
        wrapped_config = getattr(wrapped_model, "config", None)
        if hasattr(wrapped_config, "to_dict"):
            return "hf_auto", wrapped_config.to_dict()
...
def _load_model(input_dir: Path) -> tuple[PoreLM, TrainConfig]:
    ...
    if cfg.dlm.context_encoder_path is None:
        raise ValueError("config.yaml must contain dlm.context_encoder_path to rebuild PoreLM.")
...
def _write_config(output_dir: Path, model: PoreLM, cfg: TrainConfig) -> None:
    context_encoder_type, context_encoder_config = _context_encoder_hf_config(model)
    hf_config = {
        "model_type": "poredlm_dlm",
        "architectures": ["PoreLMForDiffusion"],
        "auto_map": {
            "AutoConfig": "modeling_poredlm.PoreLMConfig",
            "AutoModel": "modeling_poredlm.PoreLMForDiffusion",
        },
...
def _write_weights(output_dir: Path, model: PoreLM, safe_serialization: bool) -> None:
    ...
    if safe_serialization:
        ...
        duplicate_tied_keys = [
            "context_encoder.model.lm_head.weight",
        ]
        for key in duplicate_tied_keys:
...
def convert(
    ...
    shutil.copy2(input_dir / "config.yaml", output_dir / "porelm_train_config.yaml")

    print(f"Saved PoreLM HF checkpoint to {output_dir}")
    print("Load with the generated modeling_poredlm.py or AutoModel.from_pretrained(path, trust_remote_code=True).")


def main() -> None:
    parser = argparse.ArgumentParser(description="Convert a Stage 3 PoreLM checkpoint to a HF custom model folder.")
