...
from porelm.config import (
    CheckpointType,
    DDPGradSyncMode,
    DistributedStrategy,
    TrainConfig,
)
from porelm.data_flow_matching import build_train_flow_matching_dataloader
from porelm.eval import build_evaluators
from porelm.exceptions import PoreLMCliError, PoreLMConfigurationError
from porelm.model_porelm import PoreLM
from porelm.optim import BoltOnWarmupScheduler, build_optimizer, build_scheduler
from porelm.torch_util import (
    SingleAccelerator,
    barrier,
    get_default_device,
    get_global_rank,
...
from porelm.trainer_flow_matching import FlowMatchingTrainer
from porelm.util import (
...
def main(cfg: TrainConfig) -> None:
    ...
    if cfg.run_name is None:
        raise PoreLMConfigurationError("--run_name is required")
    ...
    if get_global_rank() == 0:
        ...
        if not cfg.dry_run and (cfg.load_path is None or Path(cfg.load_path).parent != Path(cfg.save_folder)):
            ...
            if save_path.is_file() and not cfg.save_overwrite:
                raise PoreLMConfigurationError(f"{save_path} already exists, use --save_overwrite to overwrite")
            else:
    ...
    train_loader = build_train_flow_matching_dataloader(cfg)
    ...
    if cfg.dlm.context_encoder_path is None:
        raise PoreLMConfigurationError("Stage 3 flow matching training requires `dlm.context_encoder_path` in the config.")
    porelm_model = PoreLM(
        cfg.model,
    ...
    log.info(f"Total number of parameters: {porelm_model.num_params():,d}")
    log.info(f"Number of non-embedding parameters: {porelm_model.num_params(include_embedding=False):,d}")
    ...
    log.info(f"Context hidden size: {porelm_model.context_hidden_size}")

    # Compile one block at a time.
    if cfg.compile is not None and hasattr(porelm_model, "transformer"):
        if cfg.model.block_group_size != 1:
            raise PoreLMConfigurationError("Compile is only supported with block_group_size 1.")
        for block in porelm_model.transformer.blocks:
            block.compile(**cfg.compile.asdict())

    porelm_model.set_activation_checkpointing(cfg.activation_checkpointing)
    ...
    if cfg.distributed_strategy == DistributedStrategy.ddp:
        ...
        if cfg.model.init_device != "cuda":
            raise PoreLMConfigurationError("DDP does not work with init_device set to anything other than `cuda`.")
        ...
        dist_model = DDP(porelm_model.to(device), find_unused_parameters=cfg.ddp.find_unused_params)
        ...
        wrap_policy = porelm_model.get_fsdp_wrap_policy(cfg.fsdp.wrapping_strategy)

        if version.parse(torch.__version__) >= version.parse("2.1.0") and hasattr(porelm_model, "transformer"):
        ...
        if cfg.fsdp.sharding_strategy in (ShardingStrategy.HYBRID_SHARD, ShardingStrategy._HYBRID_SHARD_ZERO2):
            if version.parse(torch.__version__) < version.parse("2.2.0"):
                ...
                raise PoreLMConfigurationError(
                    "PoreLM training does not correctly support hybrid sharding before torch 2.2.0"
                )
            ...
            if num_model_replicas <= 0:
                raise PoreLMConfigurationError("fsdp.hybrid_sharding_num_model_replicas must be a positive integer")

            if get_world_size() % num_model_replicas != 0:
                raise PoreLMConfigurationError("fsdp.hybrid_sharding_num_model_replicas must divide world size")
        ...
        param_init_fn = None
        if porelm_model is None:
            raise PoreLMConfigurationError("Model initialization failed.")
        porelm_model = porelm_model.to(device)
        dist_model = SingleAccelerator(porelm_model)

    # when param_init_fn is None, FSDP will call reset_parameters() automatically
    if param_init_fn is not None or cfg.distributed_strategy == DistributedStrategy.ddp:
        porelm_model.reset_parameters()
    ...
    if cfg.save_data_indices:
        ...
        if indices_file_path.exists() and not cfg.save_overwrite:
            raise PoreLMConfigurationError(f"{indices_file_path} already exists, use --save_overwrite to overwrite")
        indices_file_path.parent.mkdir(exist_ok=True, parents=True)
        indices_file = gzip.open(indices_file_path, "wt")

    # Consolidate components into `Trainer` object.
    with FlowMatchingTrainer(
        cfg=cfg,
        epoch=cfg.epoch,
        model=porelm_model,
...
if __name__ == "__main__":
    ...
    try:
        ...
        raise PoreLMCliError(f"Usage: {sys.argv[0]} [CONFIG_PATH] [OPTIONS]")
