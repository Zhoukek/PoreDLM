...
from hf_porelm.configuration_porelm import PoreLMConfig
from hf_porelm.modeling_porelm import PoreLMForCausalLM
from hf_porelm.tokenization_porelm_fast import PoreLMTokenizerFast
from porelm import ModelConfig, Tokenizer, TrainConfig
from porelm.checkpoint import build_sharded_checkpointer
from porelm.model import PoreLM
from porelm.util import _get_s3_client
...
def write_config(checkpoint_dir: str):
    ...
    config = PoreLMConfig(**config_kwargs)

    logger.info(f"Saving HF-compatible config to {os.path.join(checkpoint_dir, 'config.json')}")
    config.save_pretrained(checkpoint_dir)


def write_model(checkpoint_dir: str, ignore_porelm_compatibility: bool = False):
    ...
    # Loading the checkpoint using `PoreLM.from_checkpoint`` handles backwards compatibility logic.
    state_dict = PoreLM.from_checkpoint(checkpoint_dir).state_dict()
...
def convert_checkpoint(checkpoint_dir: str, ignore_porelm_compatibility: bool = False):
    print("Converting checkpoint to HF format...")
    write_config(checkpoint_dir)

    print("Saving model to checkpoint...")
    write_model(checkpoint_dir, ignore_porelm_compatibility=ignore_porelm_compatibility)
...
def fix_tokenizer(checkpoint_dir: str, tokenizer_name_or_path: Optional[str] = None):
    ...
    if tokenizer_name_or_path == "allenai/gpt-neox-porelm-dolma-v1_5" or tokenizer_name_or_path.endswith(
...
def main():
    ...
    parser.add_argument(
        "--checkpoint-dir",
        help="Location of PoreLM checkpoint.",
        required=True,
    )
    ...
    parser.add_argument(
        "--ignore-porelm-compatibility",
        action="store_true",
        help="Ignore compatibility with the porelm codebase. "
        "This will remove files that are needed specifically for porelm codebase, eg. config.yaml, etc.",
    )
    parser.add_argument(
        "--logger-level",
        default="warning",
    ...
    parser.add_argument(
        "--keep-porelm-artifacts",
        action="store_true",
        help="Keep porelm-specific artifacts in the checkpoint.",
    )
    ...
    convert_checkpoint(args.checkpoint_dir, args.ignore_porelm_compatibility)

    if not args.keep_porelm_artifacts:
