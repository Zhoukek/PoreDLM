...
from porelm.config import ActivationCheckpointingStrategy, ModelConfig
...
def create_model_config_from_pretrained_config(config: PoreLMConfig):
    """
    Utility function
    """
...
class PoreLMForCausalLM(PreTrainedModel, GenerationMixin):
    """
    ...
    _no_split_modules = ["PoreLMBlock"]
    _supports_flash_attn_2 = True
    _supports_sdpa = True
    supports_gradient_checkpointing = True

    def __init__(self, config: PoreLMConfig, model: Optional[PoreLM] = None, init_params: bool = False):
        ...
        if not model:
            ...
            self.model = PoreLM(model_config, init_params=init_params)
        else:
            self.model = model
    ...
    def forward(
        ...
        if output_attentions:
            raise ValueError("output_attentions is not yet supported in PoreLM")
    ...
    def tie_weights(self):
        ...
        See: `if not config.weight_tying: self.transformer.update(...)` in `porelm/model.py`.
    ...
    def resize_token_embeddings(
        ...
        Note:
            ...
            is less than the `vocab_size`. In PoreLM, `embedding_size` refers to the dimensionality of the model's token
...
# PoreLM is integrated directly in transformers from v4.40.0 onwards, but the version in transformers
# may not support the newest architectures we create.
AutoModelForCausalLM.register(PoreLMConfig, PoreLMForCausalLM)
