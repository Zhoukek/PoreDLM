...
def _expand_token_paths(paths: Sequence[str]) -> List[str]:
        ...
        if path_obj.is_dir():
            ...
            if not matched:
                raise PoreLMConfigurationError(
                    f"flow matching data directory {str(path_obj)!r} does not contain any .npy/.bin/.dat/.memmap token files"
                )
    ...
    if not expanded:
        raise PoreLMConfigurationError("flow matching data requires at least one file in cfg.data.paths")
    return expanded
...
class FlowMatchingTokensDataset(Dataset[Dict[str, Any]]):
    """
    Map-style token dataset for stage-3 flow matching training.

    This intentionally lives outside the original flow matching port. It reads PoreLM's
    ``cfg.data.paths`` and returns item dictionaries compatible with the flow matching
    ...
    def __init__(
        ...
        *,
        ...
        if not paths:
            raise PoreLMConfigurationError("flow matching data requires at least one path in cfg.data.paths")
        if metadata is not None and len(metadata) != len(paths):
            raise PoreLMConfigurationError(
                f"flow matching metadata has {len(metadata)} entries for {len(paths)} configured paths"
            )
        self.paths = []
        ...
        for path in self.paths:
            ...
            if array.ndim == 0:
                raise PoreLMConfigurationError(f"flow matching data file {path!r} is scalar; expected 1D or 2D token IDs")
            if array.ndim > 2:
                raise PoreLMConfigurationError(f"flow matching data file {path!r} has shape {array.shape}; expected 1D or 2D")
            index = self._load_index(path, array)
            num_instances = int(index.shape[0]) if index is not None else int(array.shape[0])
            if num_instances == 0:
                raise PoreLMConfigurationError(f"flow matching data file {path!r} has no indexed training instances")
        ...
        self._length = start
        log.info("Built flow matching token dataset with %d instances from %d file(s)", self._length, len(self.paths))
    ...
    def _normalize_instance(self, ids: np.ndarray, path: str, index: int) -> np.ndarray:
        ...
        if ids.size < 2:
            raise PoreLMConfigurationError(
                f"flow matching instance {index} in {path!r} is too short for BOS/EOS: length={ids.size}"
            )
        if int(ids[0]) != self.bos_token_id or int(ids[-1]) != self.eos_token_id:
            raise PoreLMConfigurationError(
                f"flow matching instance {index} in {path!r} must have BOS/EOS="
...
def build_train_flow_matching_dataloader(
    ...
    *,
    ...
    if train_config.data.paths is None:
        raise PoreLMConfigurationError("flow matching train dataloader currently reads cfg.data.paths")
    if train_config.data.datasets is not None:
        raise PoreLMConfigurationError("flow matching train dataloader currently expects cfg.data.paths, not cfg.data.datasets")

    max_length = _resolve_dlm_max_length(train_config)
    dataset = FlowMatchingTokensDataset(
    ...
    if get_global_rank() == 0:
        if work_dir.is_dir() and not train_config.save_overwrite:
            raise PoreLMConfigurationError(
                "flow matching train data working directory already exists, use --save_overwrite to overwrite"
            )
...
def build_eval_flow_matching_dataloader(
    train_config: TrainConfig,
    ...
    *,
    ...
    """Build an eval loader with the same sample-boundary semantics as flow matching training."""
    paths: List[str] = []
    metadata: List[Dict[str, Any]] = []
    if data_config.paths:
        if data_config.datasets:
            raise PoreLMConfigurationError("DataConfig.paths is mutually exclusive with DataConfig.datasets")
        ...
        raise PoreLMConfigurationError("One of DataConfig.paths or DataConfig.datasets is required")
    ...
    if data_config.drop_last:
        ...
        if batch_size < 1:
            raise PoreLMConfigurationError("flow matching eval dataset is too small for the distributed world size")
