...
def build_custom_dataset(train_config: TrainConfig) -> Dataset:
    ...
    :raises PoreLMConfigurationError: if the custom dataset class is not specified or the module cannot be determined
    ...
    if not train_config.data.custom_dataset.name:
        raise PoreLMConfigurationError("custom_dataset_class is required when using a custom dataset")
