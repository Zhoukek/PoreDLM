...
from .flow_matching_core.model import FLOW_MATCHING_DENOISERS
...
from .exceptions import PoreLMConfigurationError
...
__all__ = [
    ...
    "SwiGLU",
    "PoreLMBlock",
    "PoreLMSequentialBlock",
    "PoreLM",
    "PoreLMOutput",
    "PoreLMFlowMatchingOutput",
    "PoreLMGenerateOutput",
]
...
class PoreLMBlock(nn.Module):
    ...
    @classmethod
    def build(cls, layer_id: int, config: ModelConfig, cache: BufferCache) -> PoreLMBlock:
        if config.block_type == BlockType.sequential:
            return PoreLMSequentialBlock(layer_id, config, cache)
        elif config.block_type == BlockType.llama:
            return PoreLMLlamaBlock(layer_id, config, cache)
        else:
            raise NotImplementedError(f"Unknown block type: '{config.block_type}'")


class PoreLMSequentialBlock(PoreLMBlock):
    """
...
class PoreLMLlamaBlock(PoreLMBlock):
    """
    This is a transformer block where the output is computed as ``MLP(LN(x + Attention(LN(x))))``
    (plus another skip connection). This block is similar to `PoreLMSequentialBlock`
...
class PoreLMGenerateOutput(NamedTuple):
...
class PoreLMFlowMatchingOutput(NamedTuple):
    loss: torch.FloatTensor
...
class PoreLMBlockGroup(nn.ModuleList):
...
class PoreLM(nn.Module):
    def __init__(self, config: ModelConfig, init_params: bool = True):
        ...
        if self.config.alibi and self.config.flash_attention:
            raise PoreLMConfigurationError("ALiBi is currently not supported with FlashAttention")

        if self.config.alibi and self.config.rope:
            raise PoreLMConfigurationError("ALiBi and RoPE are mutually exclusive")

        if self.config.embedding_size is not None and self.config.embedding_size != self.config.vocab_size:
            if self.config.embedding_size < self.config.vocab_size:
                raise PoreLMConfigurationError("embedding size should be at least as big as vocab size")
        ...
        if not (
            ...
            raise PoreLMConfigurationError("n layers must be divisible by block group size")
        ...
        blocks = [PoreLMBlock.build(i, config, self.__cache) for i in range(config.n_layers)]
        if self.config.block_group_size > 1:
            block_groups = [
                PoreLMBlockGroup(config, i, blocks[i : i + config.block_group_size])
    ...
    def get_fsdp_wrap_policy(self, wrap_strategy: Optional[FSDPWrapStrategy] = None):
        ...
        if wrap_strategy == FSDPWrapStrategy.by_block:
            ...
            def fsdp_wrap_fn(module, recurse: bool = True, nonwrapped_numel: int = 0):
                ...
                wrap = isinstance(module, PoreLMBlock)
                if recurse:
                    return True
                else:
                    return wrap
            ...
            def fsdp_wrap_fn(module, recurse: bool = True, nonwrapped_numel: int = 0):
                del nonwrapped_numel
                wrap = isinstance(module, (PoreLMBlock,)) or module in size_based_module_to_wrap
                if recurse:
                    return True
                else:
                    return wrap
            ...
            def fsdp_wrap_fn(module, recurse: bool = True, nonwrapped_numel: int = 0):
                del nonwrapped_numel
                wrap = isinstance(module, PoreLMBlockGroup)
                if recurse:
                    return True
                else:
                    return wrap
            ...
            def fsdp_wrap_fn(module, recurse: bool = True, nonwrapped_numel: int = 0):
                del nonwrapped_numel
                wrap = isinstance(module, (PoreLMBlockGroup,)) or module in size_based_module_to_wrap
                if recurse:
                    return True
                else:
                    return wrap
            ...
            def fsdp_wrap_fn(module, recurse: bool = True, nonwrapped_numel: int = 0):
                del nonwrapped_numel
                wrap = isinstance(module, PoreLMBlock) and module.layer_id % c == 0
                if recurse:
                    return True
                else:
                    return wrap
    ...
    def from_checkpoint(
        ...
        """
        Load an PoreLM model from a checkpoint.
        """
        ...
        if checkpoint_type == CheckpointType.unsharded:
            ...
            if train_config.sharded_checkpointer == ShardedCheckpointerType.porelm_core:
                from porelm_core.distributed.checkpoint import (  # type: ignore
...
class _HFContextEncoderAdapter(nn.Module):
    """Adapts HF Stage2 context encoders to the flow matching mask/output contract."""
...
def _load_context_encoder(context_encoder_path: str) -> nn.Module:
    ...
    if isinstance(checkpoint, dict) and isinstance(checkpoint.get("model_state_dict"), dict):
        ...
        raise PoreLMConfigurationError(f"Unsupported stage2 PT checkpoint payload in {weight_path}")
...
class PoreLM(nn.Module):
    """Stage 3 flow matching: BERT encodes tokens into latents, flow matching denoises those latents."""
    ...
    def __init__(
        ...
        *,
        ...
        dlm_model_name = getattr(dlm_config, "model", "flow_matching_b")
        try:
            flow_factory = FLOW_MATCHING_DENOISERS[dlm_model_name]
        except KeyError as exc:
            raise PoreLMConfigurationError(
                f"Unknown flow matching model {dlm_model_name!r}; expected one of {sorted(FLOW_MATCHING_DENOISERS.keys())}"
            ) from exc
        self.flow_denoiser = flow_factory(
    ...
    def reset_parameters(self):
        # Flow matching denoiser and the HuggingFace BERT context encoder initialize themselves.
        # Keep this hook for compatibility with the PoreLM training script.
        return None
        ...
        **kwargs: Any,
        ...
        if bert_masked_lm:
            if attention_mask is None or cond_seq_mask is None:
                raise PoreLMConfigurationError("BERT objective requires attention_mask and cond_seq_mask")
    ...
    def bert_masked_lm_forward(
        ...
        if not bool(target_mask.any()):
            raise PoreLMConfigurationError("BERT objective batch contains no target positions")
        mask_token_id = getattr(self.context_encoder.config, "mask_token_id", None)
        if mask_token_id is None:
            raise PoreLMConfigurationError("Stage-2 BERT config does not define mask_token_id")
    ...
    def flow_matching_forward(
        ...
        if self_cond_prob > 0:
            ...
            self_cond = init_pred
        pred_x0, _ = self._flow_predict_from_latent(
            latent=denoiser_z,
            t=t,
            self_cond=self_cond,
            attention_mask=attention_mask,
    ...
    def diffusion_forward(
        ...
        del t_max, reduction
        output = self.flow_matching_forward(
            input_ids=input_ids,
            encoder_attention_mask=attention_mask,
            attention_mask=attention_mask,
            attention_bias=attention_bias,
