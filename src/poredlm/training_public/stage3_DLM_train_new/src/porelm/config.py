...
class BaseConfig:
    ...
    def new(cls: Type[C], **kwargs) -> C:
        ...
        try:
            ...
            raise PoreLMConfigurationError(str(e))
...
class DistributedStrategy(StrEnum):
    ddp = "ddp"
    """
    Wrap PoreLM in torch.nn.parallel.DistributedDataParallel to train across ranks.
    """

    fsdp = "fsdp"
    """
    Wrap PoreLM in torch.distributed.fsdp.FullyShardedDataParallel to train across ranks.
    """
...
class FSDPWrapStrategy(StrEnum):
    by_block = "by_block"
    """
    Wrap each PoreLM block with its own FSDP instance.
    """
...
class SingleGPUConfig(BaseConfig):
    ...
    def get_device(self):
        if self.device == "auto":
            ...
            raise PoreLMConfigurationError("MPS not available.")
        elif self.device == "cuda" and not torch.cuda.is_available():
            raise PoreLMConfigurationError("CUDA not available.")
        else:
...
class DLMConfig(BaseConfig):
    """
    Continuous diffusion / flow matching configuration for PoreLM stage 3.
    """
...
class TrainConfig(BaseConfig):
    ...
    """
    Distributed strategy for PoreLM model (eg. single GPU, DDP, FSDP).
    """
    ...
    """
    Deprecated, HF datasets are now stored in `porelm_data.hf_datasets`.
