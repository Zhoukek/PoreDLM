...
from porelm_data.data import get_data_path
...
def setup_logging(log_filter_type: LogFilterType = LogFilterType.rank0_only) -> None:
    ...
    if (
        os.environ.get("PoreLM_NONINTERACTIVE", False)
...
def excepthook(exctype, value, traceback):
    ...
    elif issubclass(exctype, PoreLMCliError):
        rich.get_console().print(f"[yellow]{value}[/]", highlight=False)
    elif issubclass(exctype, PoreLMError):
...
def _s3_upload(
    ...
    if not save_overwrite:
        ...
        if err is not None:
            raise PoreLMNetworkError(f"Failed to check object existence during {scheme} upload") from err

    try:
        _get_s3_client(scheme).upload_file(source, bucket_name, key)
    except boto_exceptions.ClientError as e:
        raise PoreLMNetworkError(f"Failed to upload to {scheme}") from e
...
def _s3_file_size(scheme: str, bucket_name: str, key: str, max_attempts: int = 3) -> int:
    ...
    raise PoreLMNetworkError(f"Failed to get {scheme} file size") from err
...
def _s3_get_bytes_range(
    ...
    raise PoreLMNetworkError(f"Failed to get bytes range from {scheme}") from err
...
def save_hf_dataset_to_disk(
    ...
    to `porelm_data` as follows:

    ```
    import datasets

    from porelm.util import save_hf_dataset_to_disk

    path, name, split = ...

    dataset = datasets.load_dataset(path, name=name, split=split)
    save_hf_dataset_to_disk(dataset, path, name, split, "porelm_data/hf_datasets")
    ```
    """
...
def load_hf_dataset(path: str, name: Optional[str], split: str):
    ...
    `save_hf_dataset_to_disk` and located in `porelm_data/hf_datasets`.
    """
...
def load_oe_eval_requests(path: str, name: Optional[str] = None, split: Optional[str] = None):
    """
    Loads an oe-eval request file from `porelm_data/oe_eval_tasks`.
...
def threaded_generator(g, maxsize: int = 16, thread_name: Optional[str] = None):
    ...
    for x in iter(q.get, sentinel):
        if isinstance(x, Exception):
            raise PoreLMThreadError(f"generator thread {thread_name} failed") from x
        else:
            yield x
