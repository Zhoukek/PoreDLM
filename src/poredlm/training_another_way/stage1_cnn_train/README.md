...
bash runs/continuous_cnn/run_extract_features.sh \
  save/continuous_cnn/last.pt train /path/to/features/train

bash runs/continuous_cnn/run_extract_features.sh \
  save/continuous_cnn/last.pt valid /path/to/features/valid
```

The launchers use `torchrun`. Override the GPU count when needed:

```bash
CUDA_VISIBLE_DEVICES=0,1 NPROC_PER_NODE=2 bash runs/continuous_cnn/run_train.sh
```
