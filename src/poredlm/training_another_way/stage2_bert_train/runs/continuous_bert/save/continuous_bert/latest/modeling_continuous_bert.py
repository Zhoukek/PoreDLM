...
MODEL_TYPE = "continuous_bert"


class ContinuousBertConfig(PretrainedConfig):
...
class ContinuousBert(PreTrainedModel):
    config_class = ContinuousBertConfig
    _no_split_modules = ["TransformerEncoderLayer"]

    def __init__(self, config: ContinuousBertConfig | None = None):
        config = config or ContinuousBertConfig()
...
AutoConfig.register(MODEL_TYPE, ContinuousBertConfig)
AutoModel.register(ContinuousBertConfig, ContinuousBert)
