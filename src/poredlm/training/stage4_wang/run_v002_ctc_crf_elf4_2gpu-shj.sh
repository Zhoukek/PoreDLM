...
output_dir="${OUTPUT_DIR:-${result_root}/ctc_crf_state4_bos_eos_elf4_apple_2gpu_b32_e5}"
...
env \
  ...
  WANDB_RUN_NAME="${WANDB_RUN_NAME:-V002_ctc_crf_s4_elf4_apple_2gpu_b32_e5_evalfix}" \
  BACKGROUND=0 \
  FOREGROUND_TEE="${FOREGROUND_TEE:-1}" \
  DRY_RUN="${DRY_RUN:-0}" \
  "${project_root}/run_ctc_crf_800k_4gpu-shj.sh"
