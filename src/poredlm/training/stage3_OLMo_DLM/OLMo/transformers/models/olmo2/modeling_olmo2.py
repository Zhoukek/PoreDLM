...
class CodeAwareOlmo2Attention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
    ...
    def codebook_attention_forward(
        self,
        ...
        # query_states: [B, num_q_heads, Tq, D]
        # key_states:   [B, num_kv_heads, Tk, D]
        # value_states: [B, num_kv_heads, Tk, D]

        assert key_states.size(1) == value_states.size(1)

        num_q_heads = query_states.size(1)
        num_kv_heads = key_states.size(1)

        # 处理 GQA：num_q_heads != num_kv_heads 时，需要 repeat k/v
        ...
        if code_scores is not None:
            attn_scores = attn_scores + 1.0 * code_scores.to(dtype=attn_scores.dtype)
    ...
    def forward(
        ...
        past_key_values: Optional[Cache] = None,
        cache_position: Optional[torch.LongTensor] = None,
        codeembedding: Optional[torch.Tensor] = None, # 新增的内容
        **kwargs: Unpack[TransformersKwargs],
    ) -> tuple[torch.Tensor, Optional[torch.Tensor]]:
        input_shape = hidden_states.shape[:-1]
        hidden_shape = (*input_shape, -1, self.head_dim)
        ...
        B, T, C = query_states.size()  # batch size, sequence length, d_model
        ...
        ######## 这部分是新增的内容：计算codebook的attention

        code_scores = None
        
        # layer_idx 放这里
        if codeembedding is not None and self.layer_idx in [11]:
            B, T, C = codeembedding.shape

            code_q = self.code_q_proj(codeembedding)
            code_k = self.code_k_proj(codeembedding)

            code_q = code_q.view(
                B, T, self.config.num_attention_heads, self.head_dim
            ).transpose(1, 2)
            ...
            code_scores = torch.matmul(
                code_q,
                code_k.transpose(-1, -2)
            ) * self.scaling
        ...
        if code_scores is not None:
            attn_output, attn_weights = self.codebook_attention_forward(
...
class CodeAwareOlmo2DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Olmo2Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = CodeAwareOlmo2Attention(config=config, layer_idx=layer_idx)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        hidden_states, _ = self.self_attn(
            ...
            cache_position=cache_position,
            position_embeddings=position_embeddings,
            codeembedding=codeembedding,  # 将codeembedding传递给attention
            **kwargs,
        )
        hidden_states = self.post_attention_layernorm(hidden_states)
        hidden_states = residual + hidden_states
...
@auto_docstring
class CodeAwareOlmo2PreTrainedModel(PreTrainedModel):
    config: Olmo2Config
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["CodeAwareOlmo2DecoderLayer"]
    _skip_keys_device_placement = ["past_key_values"]
    _supports_flash_attn = True
    _supports_sdpa = True
    _supports_flex_attn = True
    ...
    _supports_attention_backend = True
    _can_record_outputs = {
        "hidden_states": CodeAwareOlmo2DecoderLayer,
        "attentions": CodeAwareOlmo2Attention,
    }

@auto_docstring
class CodeAwareOlmo2Model(CodeAwareOlmo2PreTrainedModel):
    def __init__(self, config: Olmo2Config):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size
        ...
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList(
            [CodeAwareOlmo2DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = Olmo2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = Olmo2RotaryEmbedding(config=config)
        self.gradient_checkpointing = False
        
        # 从这里读取原先的codebook权重，并注册为参数
        self.codebook = nn.Parameter(torch.randn(config.vocab_size - 128, config.hidden_size))
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # 先默认使用原始 embedding
        codeembedding = inputs_embeds.clone()
        ...
        # 这里需要增加：
        # hidden_states = torch.cat((hidden_states,codeembedding), dim=2) 
...
@auto_docstring
class CodeAwareOlmo2ForCausalLM(Olmo2PreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
    _tp_plan = {"lm_head": "colwise_rep"}
    _pp_plan = {"lm_head": (["hidden_states"], ["logits"])}
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = CodeAwareOlmo2Model(config)
        self.vocab_size = config.vocab_size
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)

        # 新增加的
        # self.lm_head = nn.Linear(config.hidden_size * 2, config.vocab_size, bias=False)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ```python
        >>> from transformers import AutoTokenizer, CodeAwareOlmo2ForCausalLM

        >>> model = CodeAwareOlmo2ForCausalLM.from_pretrained("meta-olmo2/Olmo2-2-7b-hf")
        >>> tokenizer = AutoTokenizer.from_pretrained("meta-olmo2/Olmo2-2-7b-hf")
...
__all__ = ["Olmo2ForCausalLM", "Olmo2Model", "Olmo2PreTrainedModel", "CodeAwareOlmo2ForCausalLM", "CodeAwareOlmo2Model", "CodeAwareOlmo2PreTrainedModel"]
