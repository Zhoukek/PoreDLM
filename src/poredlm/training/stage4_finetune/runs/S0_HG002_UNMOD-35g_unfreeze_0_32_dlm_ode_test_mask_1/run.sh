...
input_mask_token_id=4
masked_token_ce_weight="0.0"
masked_token_ce_epochs=0
ddp_backend="nccl" 


wandb_project="stage4_finetune"
wandb_run_name="S0_HG002_UNMOD-35g_unfreeze_0_32_dlm_ode_test_tcn_28000_chunks"
...
outdir="/mnt/si002562jbsc/rnamodel/zhoukexuan/PoreDLM/src/poredlm/training/stage4_finetune/runs/S0_HG002_UNMOD-35g_unfreeze_0_32_dlm_ode_test_mask_1"

mkdir -p "${outdir}"

nohup torchrun --nproc_per_node="${nproc_per_node}" --nnodes=1 --master_port 29516 \
  ...
  --input_mask_ratio "${input_mask_ratio}" \
  --input_mask_token_id "${input_mask_token_id}" \
  --masked_token_ce_weight "${masked_token_ce_weight}" \
  --masked_token_ce_epochs "${masked_token_ce_epochs}" \
  --ddp_backend nccl \
  --save_best \
  --use_wandb \
