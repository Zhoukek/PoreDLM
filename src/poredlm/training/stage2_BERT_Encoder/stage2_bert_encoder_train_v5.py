...
def run_contiguous_mask_bert_reconstruction(
    ...
    *,
    ...
    if bwav_vocab_offset < 0:
        raise ValueError("--bwav-vocab-offset must be >= 0.")
    ...
    for start in range(0, len(codebook_ids), max_length):
        ...
        predicted_codebook_ids = predicted_vocab_ids.squeeze(0).detach().cpu().numpy() - bwav_vocab_offset
        ...
        predicted_windows.append(predicted_codebook_ids.astype(np.int64, copy=False))
    ...
    return (
        ...
        np.concatenate(predicted_windows, axis=0),
...
def save_token_id_comparison_plot(
    *,
    ...
    """Save a token-level comparison plot for stage1 ids and offset-adjusted BERT ids."""
    ...
    try:
        ...
        _save_token_id_comparison_svg(
            ...
            bwav_vocab_offset=bwav_vocab_offset,
            output_path=svg_output,
    ...
    axes[0].bar(
        ...
        width=bar_width,
        label=f"BERT token id - {bwav_vocab_offset}",
        color="#d62728",
        alpha=0.72,
    )
...
def _save_token_id_comparison_svg(
    *,
    ...
    escaped_title = html.escape(title)
    escaped_bert_label = html.escape(f"BERT token id - {bwav_vocab_offset}")
    svg = f"""<svg xmlns="http://www.w3.org/2000/svg" width="{svg_width}" height="{svg_height}" viewBox="0 0 {svg_width} {svg_height}">
  <rect width="100%" height="100%" fill="white"/>
  <text x="{left}" y="36" font-family="Arial, sans-serif" font-size="22" font-weight="700" fill="#202124">{escaped_title}</text>
  ...
  <text x="{left + 176}" y="{top + 26}" font-family="Arial, sans-serif" font-size="14" fill="#d62728">{escaped_bert_label}</text>
...
def main() -> None:
    ...
    parser.add_argument(
        "--bwav-vocab-offset",
        type=int,
        default=DEFAULT_BWAV_VOCAB_OFFSET,
        help="Vocabulary id offset used to map Stage1 codebook ids into BERT token ids.",
    )
    ...
    if args.bwav_vocab_offset < 0:
        raise ValueError("--bwav-vocab-offset must be >= 0.")
    required_vocab_size = int(args.bwav_vocab_offset) + int(stage1.codebook_size)
    if bert.config.vocab_size < required_vocab_size:
        raise ValueError(
            f"Stage2 BERT vocab_size={bert.config.vocab_size} is smaller than "
            f"bwav_vocab_offset({args.bwav_vocab_offset}) + stage1 codebook_size "
            f"({int(stage1.codebook_size)}) = {required_vocab_size}."
        )
    ...
    repaired_codebook_ids, predicted_codebook_ids, corrupted_vocab_ids, masked_positions = run_contiguous_mask_bert_reconstruction(
        bert,
        codebook_ids,
        token_mask=token_mask,
        device=device,
        codebook_size=int(stage1.codebook_size),
        bwav_vocab_offset=int(args.bwav_vocab_offset),
    ...
    bert_token_ids_minus_offset = predicted_codebook_ids
    ...
    np.savez_compressed(
        ...
        predicted_codebook_ids=predicted_codebook_ids,
        bert_token_ids_minus_offset=bert_token_ids_minus_offset,
        ...
        bwav_vocab_offset=np.asarray(args.bwav_vocab_offset, dtype=np.int64),
    ...
    output_token_plot = save_token_id_comparison_plot(
        ...
        mask_token_start=mask_token_start,
        mask_token_end=mask_token_end,
        title=f"Stage1 token id vs BERT token id - {args.bwav_vocab_offset} ({mask_token_start}:{mask_token_end})",
    )
