...
class MLPPlusCosineResidualHead:
...
def train_head(X_train, y_train, X_val, y_val, args, fold: int, model_name: str, device, torch):
    ...
    head = MLPPlusCosineResidualHead.make(torch).to(device)
...
def main() -> int:
    ...
    summary = {
        ...
        "training": {"backbone": "frozen", "head": "MLPPlusCosineResidualHead: MLP([z, abs(z)]) + sigmoid(alpha) * cosine_logit(normalize(z)); learned 2-class cosine directions and scale", "epochs_max": args.epochs, "batch_size": args.batch_size, "patience": args.patience},
