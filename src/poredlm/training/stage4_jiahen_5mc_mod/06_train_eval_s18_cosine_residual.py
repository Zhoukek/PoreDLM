...
class CosineResidualHead:
    """Cosine classifier over baseline-relative z features."""
...
def train_head(X_train, y_train, X_val, y_val, args, fold: int, model_name: str, device, torch):
    ...
    head = CosineResidualHead.make(torch).to(device)
...
def main() -> int:
    ...
    summary = {
        ...
        "training": {"backbone": "frozen", "head": "CosineResidualHead(normalize(z), learned 2-class directions, learned scale, BCE on logit_mod-logit_unmod)", "epochs_max": args.epochs, "batch_size": args.batch_size, "patience": args.patience},
