...
class L2DistancePrototypeHead:
    """L2-distance classifier using fixed positive/negative train-fold prototypes."""
    ...
    def make(torch, proto_neg: np.ndarray, proto_pos: np.ndarray):
        ...
        class Head(nn.Module):
            ...
            def forward(self, x):
                z = x[:, :768]
                proto_neg = self.proto_neg
                proto_pos = self.proto_pos
...
def train_head(X_train, y_train, X_val, y_val, args, fold: int, model_name: str, device, torch):
    ...
    head = L2DistancePrototypeHead.make(torch, proto_neg, proto_pos).to(device)
...
def main() -> int:
    ...
    for model_name, model_key in (
        ...
        for fold in range(5):
            ...
            torch.save({"state_dict": head.state_dict(), "model": model_name, "fold": fold, "head": "l2_distance_proto"}, weight_dir / f"{model_key}_fold{fold}.pt")
    ...
    summary = {
        ...
        "training": {
            "backbone": "frozen",
            "head": "L2DistancePrototypeHead: fixed train-fold positive/negative prototypes over residual z; logit = scale * (L2(z, negative prototype) - L2(z, positive prototype)) + bias",
